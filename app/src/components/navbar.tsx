...
export function Navbar() {
  ...
  const navLinks = [
    { href: '#texts', label: t('nav.texts') },
    { href: '#add-text', label: t('nav.addText') },
            ...
            <div className="hidden md:flex items-center gap-8">
              {navLinks.map((link) => (
                <a
                  key={link.href}
                  href={link.href}
                  className="relative text-sm font-medium text-lingua-text-secondary hover:text-lingua-primary transition-colors duration-200 group"
                >
                  {link.label}
                  <span className="absolute -bottom-1 left-1/2 w-0 h-0.5 bg-lingua-primary transition-all duration-300 group-hover:w-full group-hover:left-0" />
                </a>
              ))}
            </div>
                  ...
                  <div className="flex flex-col gap-4">
                    {navLinks.map((link) => (
                      <a
                        ...
                        className="text-lg font-medium text-lingua-text hover:text-lingua-primary transition-colors"
                      >
                        {link.label}
                      </a>
                    ))}
